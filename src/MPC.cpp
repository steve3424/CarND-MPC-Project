...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // VEHICLE MODEL - set up constraints for vehicle model
    // set initial constraints to initial state
	...
	fg[1 + psi_start + t] = psi1 - (psi0 - v0 * delta0 / Lf * dt); // vehicle model is - to invert psi from sim
        fg[1 + v_start + t] = v1 - (v0 + a0 * dt);
	fg[1 + cte_start + t] = cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt));	
	fg[1 + epsi_start + t] = epsi1 - ((psi0 - psides0) - v0 * delta0 / Lf * dt); // vehicle modil is - to invert psi from sim
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // set up vector with x,y values and actuations
  std::vector<double> s;
  s.push_back(solution.x[delta_start]);
  s.push_back(solution.x[a_start]);
  
  for (int i = 0; i < N; i++) {
  	s.push_back(solution.x[x_start + i]);
	s.push_back(solution.x[y_start + i]);
  } 
